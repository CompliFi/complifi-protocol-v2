...
abstract contract CollateralSplitParent is ICollateralSplit, Const {
  ...
  function split(
  ...
  ) external view virtual override returns (uint256 _split, int256[] memory _underlyingEnds) {
    ...
    (, _underlyingEnds[0], ) = iterator.getRound(
      _oracles[0],
      _settleTime,
      _underlyingEndRoundHints[0]
    );

    _split = range(splitNominalValue(normalize(_underlyingStarts[0], _underlyingEnds[0])));
  }

  function splitNominalValue(int256 _normalizedValue) public pure virtual returns (int256);

  function normalize(int256 _u_0, int256 _u_T) public pure virtual returns (int256) {
